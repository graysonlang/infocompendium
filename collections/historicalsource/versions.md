...
- **The Lurking Horror r221.870918, the sound release** (129,944 bytes). No cataloged disc ships this story file, but Masterpieces ships `LHSOUND.ZIP`, whose `LURKING.CNV` (a patch data file of the same 129,944-byte length, not itself a story file) converts the disc's r203 to r221. The repository's build matches what the converter targets by release, serial and length; whether the converter's output is byte-identical to it has not been tested (it would take running `UPDATELH.EXE` against the disc's r203 under DOS emulation).
