...
- **The Lurking Horror r221.870918, the sound release** (129,944 bytes). No cataloged disc ships this story file, but Masterpieces ships `LHSOUND.ZIP`, whose `LURKING.CNV` (a patch data file of the same 129,944-byte length, not itself a story file) converts the disc's r203 to r221. The repository carries the build the converter produces.
