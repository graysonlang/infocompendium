...
- **The Zork Zero graphics omission starts here.** `PC/ZORK0/` carries only `ZORK0.CG1` and `ZORK0.EG1` - no `.MG1` - exactly as Doherty documented for LTOI1's IBM packages, now confirmed on the CD itself. (No `.EG2` either, but per Doherty's graphics-file table Zork Zero never had one: its EGA art fits a single file. The missing file is the MCGA art alone.) The same four-file set (with `.ZIP` and interpreter) was carried byte-for-byte into Masterpieces (1996) and the Zork Legacy Collection (1997).
