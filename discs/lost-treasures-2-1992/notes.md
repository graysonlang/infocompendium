...
- **The Arthur and Journey EGA conversions were made for this disc.** `ARTHUR.EG1`/`.EG2` are dated 1992-10-22 against 1989 dates on Arthur's other files; Journey's whole DOS set except the 1989 `JOURNEY.ZIP` is stamped 1992-10-27, its `.EG1`/`.EG2` among them. These dates are two weeks before the PVD date, and the same files resurface inside Masterpieces' `DATA.Z` - including `JOURNEY.MG1`, whose 1996-06-13 stamp on Masterpieces covers a file byte-identical to this disc's 1992 copy. The timestamp stratigraphy recorded in the Masterpieces notes traces back to this mastering.
...
- **`.EG2` is the EGA overflow file.** Arthur's EGA art is split across `EG1` (125 images) and `EG2` (101 images, starting at picture 34), while the CGA and MCGA sets each fit one file (170 and 171 images) - which is why `.CG1` covers picture numbers `.EG1` lacks. Same scheme Doherty documents for the other V6 games; Shogun's fifty EGA pictures fit in one file (verified: its `EG1` covers every picture number its `CG1` has), so its lack of an `EG2` is not an omission.
