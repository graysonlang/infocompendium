...
- **`LEATHER.SCR` is present** (1,408 bytes) in `PC/LEATHER/`, and `_LEATHER.COM` is 12,004 bytes, which is Infocom MS-DOS interpreter **3N** - the only build that loads a boss key screen. So the Leather Goddesses boss key works on this disc. Doherty's Infocom Fact Sheet notes the LTOI1 IBM packages dropped it, and is internally inconsistent about the filename (`LEATHER.DAT` in one section, `<GAMENAME>.SCR` in the interpreter table). The disc settles it: `.SCR`. The file itself is not a bitmap but 1,408 bytes of plain ASCII the interpreter prints to the text screen - a fake session of **Cornerstone V5.1**, Infocom's own database product, browsing an `APPLIANCES` file of Leather Goddesses shopping advice with joke menu commands (`TOUCH`, `FONDLE`, `LUBRICATE`) and the status line "Previous Mode: Titillate". As camouflage it is worse than the game; it is one more joke wearing a boss key's clothes.
The trigger, recovered from the 3N interpreter's disassembly (2026-09-12), is **Ctrl-B**.
At a single-keystroke wait - the `[MORE]` pause, the quit confirmation - it fires immediately; at the command line the editor first echoes the keystroke as CP437's 0x02 glyph and the screen appears when Enter submits it, the same dispatcher (`cmp al, 2` on the character-consumption path) one keystroke later.
Which means that at the prompt, the command you visibly type to summon the fake spreadsheet is a smiling face.
All three routes verified live under emulation.
The display is a raw byte dump, which exposes one more period detail: the file is padded with twenty `$1a` DOS EOF bytes to a CP/M-style multiple of 128 (11 x 128 = 1,408), and the interpreter prints them too - the boss screen ends with a row of CP437 right-arrows the "database" never intended.
...
- **Zork Zero's PC graphics are incomplete.** `PC/ZORK0/` has `.CG1`, `.EG1`, `.ZIP` and the interpreter but no `.MG1`. Arthur and Journey have four graphics files apiece - but that is because their larger EGA art is split into `.EG1`/`.EG2`, not because Zork Zero is missing an `.EG2`: per Doherty's graphics-file table, Zork Zero's EGA fits one file and never had a second. The genuinely missing file is the MCGA art, the same omission Doherty documented for LTOI1's IBM packages, carried forward - and carried forward again: the 1997 Zork Legacy Collection disc ships the same four files, byte-identical, with the same gap.
...
- The V6 games keep original dates that match their builds: `ARTHUR.ZIP` is dated 1989-07-14 (its serial is 890714) and `JOURNEY.ZIP` 1989-07-06 (serial 890706), the `.CG1` hi-res art is from July 1989, and the `.EG1`/`.EG2` EGA files from 22-27 October 1992, the Lost Treasures preparation window (matching the 1992-10-29 creation date on the Mac `ARTHUR` file; per [the LTOI2 notes](../lost-treasures-2-1992/notes.md), Arthur's were generated then while Journey's are 1989 originals restamped).
